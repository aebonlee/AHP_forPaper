...
export class WorkshopService {
  ...
  private calculateConsensusAlgorithm(evaluations: any[], method: string): any {
    ...
    const individualResults = evaluations.map(eval => ({
      participantId: eval.participant_id,
      weights: eval.individual_weights,
      scores: eval.individual_scores,
      weightMultiplier: eval.weight_multiplier
    }));
